...
class Character:
    ...
    def GameLoop(self):
        ...
        if self.C_Status == "idle":
            if pygame.time.get_ticks() - self.C_Time > self.C_idle_delay:
                self.C_idle_Animation += 1
                if self.C_idle_Animation == 3:
                    self.C_idle_Animation = 0
                self.C_Time = pygame.time.get_ticks()

        elif self.C_Status == "idle_sword":
            if pygame.time.get_ticks() - self.C_Time > self.C_idle_sword_delay:
                self.C_idle_sword_Animation += 1
                if self.C_idle_sword_Animation == 4:
                    self.C_idle_sword_Animation = 0
                self.C_Time = pygame.time.get_ticks()