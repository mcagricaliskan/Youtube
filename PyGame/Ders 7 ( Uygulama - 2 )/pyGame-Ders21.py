...
def oyun():
    ...
    if Tus[pygame.K_SPACE]:
        if time.time() - atesOrani.sonZaman > atesOrani.gecikme:
            ...
            LazerSesi = pygame.mixer.music.load("Müzik/Lazer.wav")
        ...
        for Lazerler in bizimLazerList:
            if Lazerler.Kordinatlar().colliderect(Taslar.Kordinatlar()):
                try:
                    ...
                    skor.sKa += 1
                    PatlamaSesi = pygame.mixer.music.load("Müzik/Patlama.wav")
                    pygame.mixer.music.play()
...
def AnaMenu():
    ...
    FareTus = pygame.mouse.get_pressed() # 0. index

    if 850 < Fare[0] < 1050 and 750 < Fare[1] < 850:
        if FareTus[0] == 1:
            GenelDurum = "oyun"
